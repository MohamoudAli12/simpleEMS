...
def checkhealth(
    ...
    with Progress(
        ...
        getdp_env = os.environ.get("SIMPLEEMS_GETDP_BIN")
        if getdp_env and Path(getdp_env).exists():
            results.append(("getdp solver (FEM)", True, getdp_env))
        else:

...
def write_problem(
    ...
    if internal_sweep:
        ...
        xs_file = "File >"  # append one row per frequency
    else:
        k0_def = "k0[] = 2*Pi*FREQ / c0;"
        freqs_decl = ""
        resolution_op = (
        ...
        xs_file = "File"  # overwrite (single point)
    ...
    pro = f"""// Auto-generated by simpleEMS.fem_formulation for '{problem.name}'.
...
{chr(10).join(sparam_op)}
  ...
  {{ Name Get_Power ; NameOfPostProcessing postPro ;
    Operation {{
      Print [ Ploss[Domain], OnGlobal, Format Table, File StrCat[myDir, "Ploss.txt"] ] ;
      Print [ Prad[Abc], OnGlobal, Format Table, File StrCat[myDir, "Prad.txt"] ] ;
    }}
  }}
}}
"""
